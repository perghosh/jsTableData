...
export class CTableData {
    ...
    */
    ...
    static _get_data(aResult, aBody, iBegin, iEnd, options) {
        ...
        let aHidden = options.hide || null, aOrder = options.order, iSlice = options.slice || 1, bAddIndex = options.index || false;
        ...
        for (let iRow = iBegin; iRow !== iEnd; iRow += iAdd) {
            let aRow = aBody[iRow];
            aIndex.push(aRow[0]); // store physical index to row
...
CTableData.s_aJsType = [
    ["number", 196619 /* r8 */], ["string", 262160 /* str */], ["boolean", 65793 /* i1Bool */], ["date", 1048624 /* date */]
];
// BLOG
