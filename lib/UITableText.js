...
export class CUITableText {
    m_acallAction; // callback array for action hooks, like select cell, values is changing etc.
    m_acallCreate; // callback array for hooks used when items in table is created
    m_acallRender; // callback array for hooks used when items are rendered
    m_acallRenderer; // callback array used to render values, if this is set to column then internal rendering for value is disabled. Array has to match number of columns shown in table
    m_iColumnCount; // Number of columns shown, this may not match number of columns in table data
    m_aColumnFormat;
    m_aColumnPhysicalIndex; // index for column in table data
    m_aColumnPosition;
    m_eComponent; // Element that acts as container to table, sections can exist outside container but default is within
    m_oDispatch; // Dispatcher used to connect ui table to other compatible components without implement callbacks
    m_oEdits; // Component that handles edit logic
    m_sId; // Unique id for ui table
    m_aInput; // data for active input
    m_sName; // name for UI object, this can be used to make it easier to find ui class in collections
    m_iOpenEdit; // number of input elements opened.
    m_aOrder; // Order for result, if external order than pic order from  data table
    m_eParent; // parent element in DOM that owns component
    m_aRowBody; // values show in table
    m_iRowStart; // start row from table data when rows are read in to ui
    m_iRowOffsetStart; // offset  to start row, use this if 
    m_iRowCount; // Number of rows shown
    m_iRowCountMax; // Max number of rows shown
    m_aRowPhysicalIndex; // array with numbers for row index in table data object. This is to point the physical position for data
    m_oRowRows; // CRowRows is used to manage the layout for each row i CTableData
    m_aSelected; // Cell that are  selected, [row, column]
    m_bServer; // Data is stored on server, when getting new information table data is updated and default is to get all from table data
    //m_oSettings: { layout?: string };// Various settings for table text
    m_iState; // Holds internal state for UI table. State isn't used internally, this is only for external used to save some sort of state
    m_oTableData; // table source data object used to populate CUITableText
    m_oTableDataTrigger; // Trigger logic for table
    m_aSection; // Sections
    m_oStyle;
    m_eSupportElement;
    m_aValueError; // Error values
    m_oWidth;
    static s_sWidgetName = "uitabletext";
    static s_iIdNext = 0;
    /**
     * Default styles
     */
