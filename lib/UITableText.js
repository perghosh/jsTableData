...
export class CUITableText {
    ...
    SetCellValue(_Row, _Column, value, oTriggerData) {
        ...
        if (_result === true || _result[0] === true) {
            ...
            if (oTrigger) {
                ...
                bOk = oTrigger.Trigger(65556 /* BeforeSetCellError */, oTriggerData, oTriggerData.edit.GetValueStack());
                if (bOk === false)
                    return;
            }
    ...
    Sort(_Sort, bToggle, bAdd) {
        ...
        if (oTrigger) {
            bOk = oTrigger.Trigger(65552 /* BeforeSetSort */, { iReason: 4 /* Command */, dataUI: this }, arguments);
            if (bOk === false)
                return;
        }
        ...
        if (oTrigger) {
            bOk = oTrigger.Trigger(131089 /* AfterSetSort */, { iReason: 4 /* Command */, dataUI: this }, arguments);
        }
    }
    Destroy() {
        if (this.m_eComponent) {
    ...
    */
    ...
    render_body(_1, eSection) {
        ...
        if (typeof _1 === "boolean") {
            let _Selected = CTableData.GetPropertyValue(this.m_oStyle, false, "class_cell_input") || CTableData.GetPropertyValue(this.m_oStyle, false, "class_selected");
            let sClassSelected = CTableData.GetPropertyValue(this.m_oStyle, false, "class_cell_input") || CTableData.GetPropertyValue(this.m_oStyle, false, "class_selected");
            if (_Selected)
        ...
        this.m_aRowBody.forEach((aRow, iIndex) => {
            ...
            if (eR.dataset.type !== "row") {
                eR = eR.querySelector('[data-type="row"]');
            ...
            for (var i = 0; i < this.m_iColumnCount; i++) {
                ...
                if (!oPosition.row) { // place value on record row ?
                    ...
                    if (bCall)
                        this.m_acallRender.forEach((call) => { call.call(this, "afterCellValue", sValue, eColumn, oPosition); });
    ...
    create_body(aBody) {
        ...
        let _HtmlExtraRow = CTableData.GetPropertyValue(this.m_oStyle, false, "html_row_body_extra") || _HtmlRow;
        ...
        let aRows = [aRow]; // rows for each record. One recod could have column values in many rows, thats why rows are placed in array
    ...
    _create_row_extra(aRow, oPosition) {
        if (!Array.isArray(aRow))
            aRow = [aRow];
        let eRoot = null, eRow, eParent;
        for (let i = 0; i < aRow.length; i++) {
