...
export class CTableDataTrigger {
    constructor(options) {
        ...
        this.m_aTrigger = (new Array(30 /* LAST_EVENT */)).fill(true);
        this.m_acallTrigger = [];
        if (o.trigger)
    ...
    static SetTriggerName(aName) {
        if (CTableDataTrigger.s_aTriggerName.length === 0)
            CTableDataTrigger.s_aTriggerName = new Array(30 /* LAST_EVENT */);
    ...
    ObserveResize(_1, bAdd) {
        ...
        if (!this.m_oRS) {
            this.m_oRS = new window.ResizeObserver((aE, oRS) => {
                ...
                for (let _e of aE) {
                    this.TriggerOn(this.Call([262167 /* OnResize */]), e, _e);
                }
            });
        }
