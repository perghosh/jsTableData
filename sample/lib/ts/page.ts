import { CTableData, CRowRows, enumMove, IUITableData, tabledata_column, tabledata_position, tabledata_format, enumReturn } from "TableData.js";
import { edit } from "TableDataEdit.js";
import { CTableDataTrigger, EventDataTable } from "TableDataTrigger.js";


import { CUITableText, uitabledata_construct } from "UITableText.js"
...
export class CPage {
   ...
   IsReadyToVote( bUpdateVoteButton?: boolean ) {
      ...
      if(bUpdateVoteButton === true) {
         let e = <HTMLElement>document.getElementById("idPollQuestionList").querySelector('[data-section="vote"]').querySelector("button");
...
*/
   ...
   RESULTCreatePollList(eRoot: string|HTMLElement, oResult: any) {
      ...
      if(aBody[ 0 ].length) {
         ...
         eList.addEventListener('change', e => {
            ...
            if(isNaN(iPoll)) this.SetActivePoll(-1);
            else {
   ...
   RESULTCreateQuestionPanel(eRoot: string|HTMLElement, oResult: any) {
      ...
      let eQuestion = <HTMLElement>eRoot.querySelector('[data-section="question"]');
      if(!eQuestion) {
